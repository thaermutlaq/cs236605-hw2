...
class Linear(Block):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        # ====== YOUR CODE: ======
        self.w = torch.randn(out_features, in_features).normal_(mean=0, std=wstd)
        self.b = torch.ones(out_features).normal_(mean=0, std=wstd)
        self.grad_cache['x'] = []
        # ========================
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = torch.matmul(x, self.w.transpose(dim0=0,dim1=1)) + self.b
        # ========================
    ...
    def backward(self, dout):
        ...
        self.dw += torch.matmul(x.transpose(dim0=0,dim1=1), dout).transpose(dim0=0,dim1=1)
...
class CrossEntropyLoss(Block):
    ...
    def backward(self, dout=1.0):
        ...
        """
        x = self.grad_cache['x'].pop()
        y = self.grad_cache['y'].pop()
        N = x.shape[0]

        # TODO: Calculate the gradient w.r.t. the input x
        # ====== YOUR CODE: ======
        dx = torch.exp(x) / torch.sum(torch.exp(x), dim=1).view(N ,1)
        dx[range(N), y] -= 1
        dx = dx / N
        dx = dout * dx
        # ========================
...
class Dropout(Block):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        d_m = torch.ones_like(x)
        out = x
        if self.training_mode:
            d_m = torch.bernoulli(d_m*(1-self.p))
            out *= d_m
        else:
            out *= (1-self.p)
        self.grad_cache['d_m'] = d_m
        # ========================
