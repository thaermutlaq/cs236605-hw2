...
class Trainer(abc.ABC):
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            # ====== YOUR CODE: ======
            epoch_loss, epoch_acc = self.train_epoch(dl_train, **dict(kw, verbose=verbose))
            train_loss += epoch_loss
            train_acc.append(epoch_acc)
            
            valid_loss, valid_acc = self.test_epoch(dl_test, **dict(kw, verbose=verbose))
...
class BlocksTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        #backward
        dz = self.loss_fn.backward(loss)
        self.model.backward(dz)
        #grad
        self.optimizer.step()
        #calc
        values, indices = y_pred.max(1)
        num_correct = torch.sum(y == indices)
        # ========================
