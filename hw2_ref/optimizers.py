...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.vs = [torch.zeros_like(param[0]) for param in params]
        # ========================

    def step(self):
        for (p, dp), v in zip(self.params, self.vs):
            if dp is None:
                continue
            ...
            # ====== YOUR CODE: ======
            v *= self.momentum
            v -= self.learn_rate*(dp + self.reg*p)
            p += v
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.rs = [torch.zeros_like(param[0]) for param in params]
        # ========================

    def step(self):
        for (p, dp), r in zip(self.params, self.rs):
            if dp is None:
                continue
            ...
            # ====== YOUR CODE: ======
            r *= self.decay
            r += (1-self.decay)*(dp + self.reg*p)**2
            p -= (self.learn_rate/torch.sqrt(r+self.eps))*(dp + self.reg*p)
            # ========================
