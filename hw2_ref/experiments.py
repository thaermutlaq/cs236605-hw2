...
def run_experiment(run_name, out_dir='./results', seed=None,
    ...
    fit_res = None
    # ====== YOUR CODE: ======
    filters = [x for x in filters_per_layer for y in range(layers_per_block)]
    X, _ = ds_train[0]
    model = model_cls(in_size = X.shape, out_classes = 10,filters=filters ,pool_every=pool_every, hidden_dims=hidden_dims)
    loss_fn = torch.nn.CrossEntropyLoss()
    optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=reg, nesterov=True)
